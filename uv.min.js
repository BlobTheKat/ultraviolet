...
K.delete()}}delete(){const{j}=this;if(j){g.deleteFramebuffer(j);if(j.X)g.deleteRenderbuffer(j.X);if(j.Y)g.deleteRenderbuffer(j.Y)}}}N.setTargetCanvas=z=>{g=z.getContext('webgl2',{preserveDrawingBuffer:0,antialias:0,depth:0,premultipliedAlpha:1,S:1});g.pixelStorei(37440,1);C=g.createBuffer();g.bindBuffer(34962,C)
