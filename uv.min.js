...
if(this.B)g.deleteBuffer(this.B)};const I=new F(6);class K{j;p;m;#a;#b;#c;#d;#e;#f;get width(){return(this.j||g.canvas).width}get height(){return(this.j||g.canvas).height}get texture(){return this.j?.t}getData(x=0,y=0,w=this.width,h=this.height,a=null){if(j!=(j=this.j))g.bindFramebuffer(36160,j);const f=j?j.f:D
...
if(W|H){if(S!=W||T!=H)g.viewport(0,0,S=W,T=H);g.stencilMask(q=255);g.clear(1024)}}}draw(K,e,u8,u9,u0,u1,u2,u3,D=1e999){if(j!=(j=this.j))g.bindFramebuffer(36160,j);{const W=(j||g.canvas).width,A=(j||g.canvas).height;if(!W|!A)return;if(S!=W||T!=A)g.viewport(0,0,S=W,T=A)}if(E!=(E=this.p))g.useProgram(E),a=H=NaN;I[0]=this.#a*2;I[3]=this.#b*2;I[1]=this.#c*2;I[4]=this.#d*2;I[2]=this.#e*2-1;I[5]=this.#f*2-1;g.uniformMatrix2x3fv(E.m,0,I);if(a!=u0||c!=u1||d!=u2||f!=u3)g.uniform4f(E.a,a=u0,c=u1,d=u2,f=u3);if(H!=u8)g.uniform1f(E.b,H=u8);if(J!=u9)g.uniform1f(E.c,J=u9);const m=this.m,s=j?j.S:O,Q=L^m^(s!=q)<<4,b=this.j?.t
...
g.uniform1i(E.t[0],U)}y=+!e.u}if(b)b.m|=1;if(K instanceof WebGLVertexArrayObject){if(G!=K)g.bindVertexArray(G=K);g.drawArraysInstanced(5,0,4,Math.min(D,K.size));return}g.bindBuffer(34962,C);if(G)g.bindVertexArray(G=null);if(K.byteLength){g.bufferData(34962,K,35040)
...
K.delete()}}delete(){const{j}=this;if(j){g.deleteFramebuffer(j);if(j.X)g.deleteRenderbuffer(j.X);if(j.Y)g.deleteRenderbuffer(j.Y)}}}N.setTargetCanvas=z=>{g=N.gl=z.getContext('webgl2',{preserveDrawingBuffer:0,antialias:0,depth:0,premultipliedAlpha:1,stencil:1});g.pixelStorei(37440,1);C=g.createBuffer();g.bindBuffer(34962,C)
