...
if(this.B)g.deleteBuffer(this.B)};const I=new F(6);class K{j;#a;#b;#c;#d;#e;#f;#x;#y;#z;#w;#s;#t;get width(){return(this.j||g.canvas).width}get height(){return(this.j||g.canvas).height}get texture(){return this.j?.t}getData(x=0,y=0,w=this.width,h=this.height,a=null){if(j!=(j=this.j))g.bindFramebuffer(36160,j);const f=j?j.f:D
...
this.#y=b;this.#z=c;this.#w=d}setST(s=0,t=0){this.#s=s;this.#t=t}clear(r=0,g=0,b=0,a=0){g.clearColor(r,g,b,a);L=(L&240)|(r==r)|(g==g)<<1|(b==b)<<2|(a==a)<<3;g.colorMask(L&1,L&2,L&4,L&8);const x=this.j,q=x?x.S=x.S+1&7:O=O+1&7;if(j!=x)g.bindFramebuffer(36160,j=x);const W=(j||g.canvas).width,H=(j||g.canvas).height;if(W|H){if(S!=W||T!=H)g.viewport(0,0,S=W,T=H);g.clear(q?16384:(g.stencilMask(255),17408))}g.stencilMask(1<<q)}clearColor(r=0,g=0,b=0,a=0){g.clearColor(r,g,b,a);L=(L&240)|(r==r)|(g==g)<<1|(b==b)<<2|(a==a)<<3
g.colorMask(L&1,L&2,L&4,L&8);if(j!=(j=this.j))g.bindFramebuffer(36160,j);const W=(j||g.canvas).width,H=(j||g.canvas).height;if(W|H){if(S!=W||T!=H)g.viewport(0,0,S=W,T=H);g.clear(16384)}}clearStencil(){const x=this.j,q=x?x.S=x.S+1&7:O=O+1&7;if(!q){if(j!=x)g.bindFramebuffer(36160,j=x);const W=(j||g.canvas).width,H=(j||g.canvas).height;if(W|H){if(S!=W||T!=H)g.viewport(0,0,S=W,T=H);g.stencilMask(255);g.clear(1024)}}g.stencilMask(1<<q)}draw(K,e,m=15,k=1135889,D=1e999){if(j!=(j=this.j))g.bindFramebuffer(36160,j);{const W=(j||g.canvas).width,A=(j||g.canvas).height
if(!W|!A)return;if(S!=W||T!=A)g.viewport(0,0,S=W,T=A)}if(E!=(E=this.p))g.useProgram(E),a=H=NaN;I[0]=this.#a*2;I[3]=this.#b*2;I[1]=this.#c*2;I[4]=this.#d*2;I[2]=this.#e*2-1;I[5]=this.#f*2-1;g.uniformMatrix2x3fv(E.m,0,I);if(a!=this.#x||c!=this.#y||d!=this.#z||f!=this.#w)g.uniform4f(E.a,a=this.#x,c=this.#y,d=this.#z,f=this.#w);if(H!=(H=this.#s))g.uniform1f(E.b,H);if(J!=(J=this.#t))g.uniform1f(E.c,J);m=m&0x880000ff|k<<8
if((L^m)&15)g.colorMask(m&1,m&2,m&4,m&8);if((L^m)&240){if(m&240){const s=j?j.S:O;if(!(L&240))g.enable(2960);if((m^L)&240){g.stencilFunc(m&32?m&16?512:517:m&16?514:519,255,1<<s);const o=m&128?m&64?5386:7681:m&64?0:7680;g.stencilOp(o,o,o)}}else if(L&240)g.disable(2960)}if((L^m)&0x77000000)g.blendEquationSeparate((m>>24&7)+32773,(m>>28&7)+32773)
if((L^m)&16776960)g.blendFuncSeparate((m>>8&15)+766*!!(m&3584),(m>>16&15)+766*!!(m&917504),(m>>12&15)+766*!!(m&57344),(m>>20&15)+766*!!(m&14680064));if((m^L)&0x8000000)m&0x8000000?g.enable(3024):g.disable(3024);L=m;const b=this.j?.t;if(Array.isArray(e)){if(e.length<E.t.length||e.length>16)return z(4,'.draw(): Shader expects '+E.t.length+' texture(s)');let v=65535,i=0;for(;i<e.length;i++){const t=e[i]
...
K.delete()}}delete(){const{j}=this;if(j){g.deleteFramebuffer(j);if(j.X)g.deleteRenderbuffer(j.X);if(j.Y)g.deleteRenderbuffer(j.Y)}}}N.setTargetCanvas=z=>{g=N.gl=z.getContext('webgl2',{preserveDrawingBuffer:0,antialias:0,depth:0,premultipliedAlpha:1,S:1});g.pixelStorei(37440,1);C=g.createBuffer();g.bindBuffer(34962,C)
